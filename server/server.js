...
// ─── GEMINI VISION AI PROXY ───
// Keeps the API key server-side; frontend never sees it.
// Fallback chain — tries multiple models in order of preference.
const GEMINI_MODELS = [
...
async function callGeminiModel({ version, model }, key, base64Image, mimeType) {
...
app.post('/api/gemini/analyze', async (req, res) => {
  ...
  for (const attempt of GEMINI_MODELS) {
    try {
      console.log(`[Gemini Proxy] Trying ${attempt.version}/${attempt.model}`);
      const geminiRes = await callGeminiModel(attempt, GEMINI_KEY, base64Image, mimeType);

      if (!geminiRes.ok) {
        const errText = await geminiRes.text();
        console.error(`[Gemini Proxy] ${attempt.version}/${attempt.model} failed (${geminiRes.status}):`, errText);
        lastError = `${attempt.version}/${attempt.model} → HTTP ${geminiRes.status}`;
        continue;
      }
      ...
      try {
        ...
        console.log(`[Gemini Proxy] Success with ${attempt.version}/${attempt.model}`);
      ...
      console.error(`[Gemini Proxy] Fetch error (${attempt.version}/${attempt.model}):`, err.message);
